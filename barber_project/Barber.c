...
// creating finish and receipt array for customers.
...
// pthread_create(&threads[i],NULL,(void *)&cashcounter,(void *)i);
 ...
 printf("Please enter c for creating customers\n");
 i=0;k=1;
 while(k=1) {
  scanf("%c",&ch);
  if(ch=='c' && i<50){
    ...
    printf("Curtomers have reached to limit.\n");
  }
  else if(ch!='c'){
    printf("You pressed %c not c\n",ch );
  }
  else{
    printf("else working..\n");
  }
